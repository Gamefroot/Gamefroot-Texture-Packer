	...
	function getNextFileData () {
		if ( !filePaths.length ) {
			...
			sizes = sizes.filter( function( size ){
				let squareBracketIndex = size.indexOf( "[" );
				let filePath = size.substring( 0, squareBracketIndex );
				let uniqueFilePath = filePath !== lastFilePath;
				lastFilePath = filePath;
				return uniqueFilePath || squareBracketIndex === -1;
			});
...
exports.generateImage = function (files, options, callback) {
	...
	var MAX_COMMAND_SIZE = Math.pow( 2, 16 );

	function execLog ( command, callbackFinal ) {
		// Wrap an exec command in logs so we can see what it's doing
		// and what its outputs are, if any.
		return exec( command, function ( err, stdout, stderr ) {
			console.log( "PACKER:generateImage()", "building", outputPath );
			if ( stdout ) {
				console.log( "PACKER:generateImage()", "stdout\n", stdout );
			}
			if ( stderr ) {
				console.log( "PACKER:generateImage()", "stderr\n", stderr );
			}
			callbackFinal( err );
		} );
	}
	...
	addOperation( "convert -limit memory 512MiB -define png:exclude-chunks=date -quality 0% -size " + options.width + "x" + options.height + " xc:none" );
	...
	async.eachSeries(files, function (file, callbackAsync) {
		...
		if (commandSize > MAX_COMMAND_SIZE) {
			...
			command = [];
			commandSize = 0;
			addOperation( "convert -limit memory 512MiB \"" + outputPath + "\"" );

			// Execute command.
			return execLog(cmd, callbackAsync);
		} else {
		...
		if ( command.length ) {
			...
			command.push( '"' + outputPath + '"' );
			var cmd = command.join( " " );
			return execLog( cmd, function ( err ) {
				if ( err ) {
					return callback( err );
				}
